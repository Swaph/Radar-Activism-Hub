...
export default function PublicRoom() {
    ...
    const isMod = moderators[roomId]?.includes(username);

    useEffect(() => {
        if (!roomId) navigate(`/public/${crypto.randomUUID()}`);
    }, [roomId, navigate]);
    ...
    }, [roomId, username, setPeers, setConnectionStatus]);
    ...
    return (
        <div className="wf-main">
            ...
            <h2 className="wf-room-title">
                Public Room: <span className="wf-room-id">{roomId}</span>
            </h2>
                ...
                <CardContent className="wf-chatbox__content">
                    {messages.map((msg) => {
                        ...
                        const handleReaction = (emoji) => {
                            ...
                            if (userHasReactedToAny && !alreadyReactedToThis) return; // only 1 emoji allowed
                                                ...
                                                <>
                                                    ...
                                                    {(isOwn || isMod) && (
                                                        <IconButton
                                                            onClick={() => {
                                                                if (!isOwn && isMod) {
                                                                    if (window.confirm(`Delete ${msg.sender}'s message as moderator?`)) {
                                                                        deleteMessage(msg.id, true); // true indicates mod deletion
                                                                    }
                                                                } else {
                                                            ...
                                                            size="small"
                                                            color={!isOwn && isMod ? "error" : "default"}
                                                            title={!isOwn && isMod ? "Delete as moderator" : "Delete message"}
                                                        >
